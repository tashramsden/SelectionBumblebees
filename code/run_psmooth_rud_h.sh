...
mkdir $smooth_path/rud_h_smooth_sigma$label


## rud - vcf - smoothed stats - first run - generate bootstrap archive
echo -e "start vcf rud h smooth: `date`" >> $smooth_path/stopwatch.txt
/rds/general/user/ter21/home/stacks-2.62/populations \
	...
	--out-path $smooth_path/rud_h_smooth_sigma$label	`# output dir`
echo -e "end vcf rud h smooth: `date`" >> $smooth_path/stopwatch.txt

## second run - use bootstrap archive to bootstrap resample
echo -e "start vcf rud h smooth 2 w 10000 reps: `date`" >> $smooth_path/stopwatch.txt
/rds/general/user/ter21/home/stacks-2.62/populations \
	--in-path $path/refmap_hort_178n 		`# input dir - stacks aligned to hort ref genome` \
	--popmap $rud_popmap					`# popmap` \
	--threads 32 							`# num threads` \
	...
	--vcf-all									`# output results in VCF format` \
	--out-path $smooth_path/rud_h_smooth_sigma$label	`# output dir`
echo -e "end vcf rud h smooth 2: `date`" >> $smooth_path/stopwatch.txt
