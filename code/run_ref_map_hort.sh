...
echo -e "178n Start date: `date`" >> ref_map_outputs/stopwatch.txt  # start time
ref_map.pl \
    -T 32 `# num CPUs` \
    --samples align_seqs_bwa/aligned_to_hort_bam/ `# location of BAM files` \
    --popmap metadata/popmap_178n_spp.csv `# population map file: sample names to be used and their species` \
    -o ref_map_outputs/refmap_hort_178n/ `# path to output file directory`
