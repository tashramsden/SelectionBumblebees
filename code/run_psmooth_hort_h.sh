...
mkdir $smooth_path/hort_h_smooth_sigma$label


## hort - vcf - smoothed stats - first run - generate bootstrap archive
echo -e "start vcf hort smooth: `date`" >> $smooth_path/stopwatch.txt
/rds/general/user/ter21/home/stacks-2.62/populations \
	...
	--out-path $smooth_path/hort_h_smooth_sigma$label	`# output dir`
echo -e "end vcf hort smooth: `date`" >> $smooth_path/stopwatch.txt

## second run - use bootstrap archive to bootstrap resample
echo -e "start vcf hort smooth 2 w 10000 reps: `date`" >> $smooth_path/stopwatch.txt
/rds/general/user/ter21/home/stacks-2.62/populations \
	--in-path $path/refmap_hort_178n 		`# input dir - stacks aligned to hort ref genome` \
	--popmap $hort_popmap					`# popmap` \
	--threads 32 							`# num threads` \
	...
	--vcf-all									`# output results in VCF format` \
	--out-path $smooth_path/hort_h_smooth_sigma$label	`# output dir`
echo -e "end vcf hort smooth 2: `date`" >> $smooth_path/stopwatch.txt
